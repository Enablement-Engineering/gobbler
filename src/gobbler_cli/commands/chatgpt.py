"""ChatGPT interaction commands via the Gobbler browser extension."""
...
app = typer.Typer(help="Interact with ChatGPT via Gobbler extension")
...
def chatgpt_callback(
    ...
    """Interact with ChatGPT via Gobbler extension."""
    set_auto_start(not no_auto_start)


# JavaScript snippets for ChatGPT interaction
# These use the injected window.gobblerChatGPT API when available, with fallbacks

# Check if page API is available
CHECK_API_JS = "typeof window.gobblerChatGPT !== 'undefined'"
...
(() => {
    if (typeof window.gobblerChatGPT !== 'undefined') {
        return window.gobblerChatGPT.getConversationInfo();
    }
    // Fallback
    const title = document.title || "ChatGPT";
    const url = window.location.href;
    const conversationId = url.match(/\\/c\\/([^/?]+)/)?.[1] || null;
    return { title, url, conversationId, isConversation: url.includes('/c/') };
})()
"""
...
(async () => {
    if (typeof window.gobblerChatGPT !== 'undefined') {
        return await window.gobblerChatGPT.getLastResponse();
    }
    // Fallback
...
(async () => {
    if (typeof window.gobblerChatGPT !== 'undefined') {
        const result = await window.gobblerChatGPT.getChatContent();
        if (result.success) {
            const count = %s;
    ...
    return { error: 'Gobbler ChatGPT API not available' };
})()
"""


async def _get_chatgpt_tabs() -> list[dict[str, Any]]:
...
async def _list_conversations() -> None:
    ...
    tabs = await _get_chatgpt_tabs()

    if not tabs:
        print_info("No ChatGPT tabs found in Gobbler group")
        console.print("\nTo use ChatGPT with Gobbler:")
        console.print("  1. Open ChatGPT in your browser")
...
def info(
    ...
    """Get information about a ChatGPT conversation."""
...
async def _info(tab_id: int | None) -> None:
    ...
    tabs = await _get_chatgpt_tabs()
    if not tabs:
        print_error("No ChatGPT tabs found in Gobbler group")
        raise typer.Exit(1)
...
@app.command()
def query(
    message: Annotated[str, typer.Argument(help="Message to send to ChatGPT")],
    tab_id: Annotated[
        int | None,
        typer.Option("--tab", "-t", help="Specific tab ID"),
    ] = None,
...
async def _query(  # noqa: C901, PLR0912, PLR0915
    ...
    with ProgressTracker("Waiting for ChatGPT response"):
    ...
    if isinstance(data, dict):
        if data.get("success"):
            ...
            if data.get("partial"):
                ...
                print_info("Use 'gobbler chatgpt last' to check for complete response")
        else:
...
def last(
    ...
    """Get the last response from ChatGPT chat."""
...
def history(
    ...
    """Get recent messages from ChatGPT chat history."""
...
def download(
    ...
    """Download images from the last ChatGPT response."""
...
async def _download(  # noqa: C901, PLR0912, PLR0915
    ...
    for i, img_url in enumerate(images, 1):
        ...
        try:
            ...
            # Save to file
            filename = f"chatgpt_image_{i}.png"
