...
When using JSON modes, parse stdout as data. `gobbler doctor --json`, `gobbler status --json`, and conversion commands with `--format json` write one JSON object to stdout without progress text. `gobbler status --json` exits nonzero when the reported overall status is degraded, while still writing the diagnostic JSON body. Batch commands with `--json` write newline-delimited JSON events; queued batch webpage submissions with `--queue --json` write a single `job_queued` or `queue_error` object. Documented CLI JSON objects include top-level `schema_version: 1`; batch `batch_complete` events include `summary.total`, `summary.successful`, `summary.failed`, and `summary.skipped`.
